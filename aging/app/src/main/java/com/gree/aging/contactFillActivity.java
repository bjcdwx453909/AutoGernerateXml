...
public class contactFillActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private ProgressWheel contactfillprogressWheel;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_contact_fill);
        IntentFilter filter=new IntentFilter();
        filter.addAction("CONTACTFILL.TEST");
        contactFillRevice contactFillRevice = new contactFillRevice();
        registerReceiver(contactFillRevice,filter);
        fillCountEt = findViewById(R.id.contactfillCountEt);
        fillCountTV = findViewById(R.id.contactfillCountTV);
        startFillBtn = findViewById(R.id.contactfillStartBtn);
        stopFillBtn = findViewById(R.id.contactfillStopBtn);
        Button delBtn = findViewById(R.id.contactDelBtn);
        contactfillprogressWheel = findViewById(R.id.contactfillprogressWheel);
        startFillBtn.setOnClickListener(this);
        stopFillBtn.setOnClickListener(this);
        delBtn.setOnClickListener(this);
        final RxPermissions rxPermissions = new RxPermissions(this);
        rxPermissions
                .requestEach(Manifest.permission.READ_CONTACTS, Manifest.permission.WRITE_CONTACTS)
                .subscribe(permission -> {
                    if(!permission.granted ||permission.shouldShowRequestPermissionRationale) permissionFlag=false;
                });
        handler = new Handler() {
            @Override
            public void handleMessage(@NonNull Message msg) {
                switch (msg.what) {
                    ...
                    case 99:
                        ...
                        setTestPercentage();
                        Toast.makeText(getApplicationContext(), "联系人填充完成", Toast.LENGTH_SHORT).show();
                        break;
    ...
    public boolean getFillPecent() {
        ...
        if (fillPecentValue.equals("")) {
            fillCount = 1000;
            Toast.makeText(this, "默认填充1000个联系人", Toast.LENGTH_SHORT).show();
            return true;
        }
        try {
    ...
    public void addContact() {
        String name = getRandomName(2, 4);
        String phoneNumber = createMobile();
        String address = getRoad();
        String Email = getEmail(3, 8);
        // 创建一个空的ContentValues
...
//        param op 0 移动 1 联通 2 电信
    ...
    public void setTestPercentage() {
        contactfillprogressWheel.setStepCountText(fillProgress + "%");
        contactfillprogressWheel.setPercentage(fillProgress * 360 / 100);
        Log.w(TAG,String.format("填充联系人:%s", fillProgress > 100 ? 100 : fillProgress));

//        writeProcess(String.format("填充联系人:%s", fillProgress > 100 ? 100 : fillProgress));
    }
    private void writeProcess(){
        ...
        try{
            ...
            fileWritter.write("填充联系人:"+ "填充联系人:100");
            fileWritter.close();
        }
        catch (Exception e){
...
//                    writeProcess();
    ...
    class contactFillRevice extends BroadcastReceiver {
